    ...
    <div class="col-md-8">
        <form method="POST" action="{{ route('recetas.update', ['receta' => $receta->id])}}"  novalidate enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="titulo">Título Receta {{$receta->titulo}}</label>
                <input type="text" class="form-control @error('titulo') is-invalid @enderror" name="titulo" id="titulo" value=" {{ $receta->titulo}}" placeholder="Título Receta">
                @error('titulo')
